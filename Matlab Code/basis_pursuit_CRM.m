function [z, history] = basis_pursuit_CRM(A, b, rho, alpha)
...
for k = 500:MAX_ITER
    ...
    %having updated x one extra time, I can update all my reflections and
    %true dual
    ...
    RRy = u -rho*z+2*rho*x; %I have removed A here because the matrix for the basis pursuit problem is Id. Must check.
    ...
    if colinear_check(DRy,Ry,RRy,10^(-3)) == 1 %I check for colinearity
		center = circumcenter(DRy,Ry,RRy); %I take the circumcenter of the dual
        u = proj_box(center,1); %I take the shadow of the centered dual
    else
        u = u + (x_hat - z); % If colinear, I do the regular multiplier update
    end
